...
public class IndonesiaBatchGraphicsScreenshot {
    public static void main(String[] args) {
        // 记录起始时间
        long l = System.currentTimeMillis();

        String filePath = "E:\\Jae\\sibu\\需求1\\印尼\\101-ENR-3.4.html"; // 替换HTML文件路径
//        String filePath = "E:\\Jae\\sibu\\需求1\\泰国\\VT-ENR-3.1-en-GB.html"; // 替换HTML文件路径
            ...
            while (true) {
                ...
                for (Element imageElement : imageElements) {
                    ...
                    if (imageElements.size() > 20) {
                        ...
                        screenShot(pageIndex, imageLeft, imageTop, imageWidth, imageHeight);
                    }
     ...
     * @param imageHeight
     */
    private static void screenShot(int pageIndex, String imageLeft, String imageTop, String imageWidth, String imageHeight) {

        try {
            // 1. 加载PDF文档
            PDDocument document = PDDocument.load(new File("E:\\Jae\\sibu\\需求1\\印尼\\101-ENR 3.4.pdf"));
...
//            int pageIndex = 0;
            ...
            float topCoefficient = -4f;
            float widthCoefficient = 14f;
            float heightCoefficient = 14f;
...
//          System.out.println(ptCommon);
            ...
            if (x<0 || y<0){
//                x = 0;// 可以截，但是缺角，原图也是缺角，left=0;top=4
...
//            float x = left * ptCommon; // 起始X坐标 系数：4.1666
//            float y = top * ptCommon; // 起始Y坐标 系数：4.1666
//            float w = width * ptCommon; // 宽度 系数：4.1666
//            float h = height * ptCommon; // 高度 系数：4.1666
            ...
            if (isScreenshotFlag){
                ...
                // 7. 保存截图为PNG格式的图片文件
                File outputFile = new File("E:\\Jae\\sibu\\需求1\\印尼\\印尼所有图形截图\\印尼ENR3所有图形截图\\101-ENR-3.4所有页图形图片\\" + "101-ENR-3.4" + "-Page" + pageIndex
                        + " imageLeft-" + imageLeft + " imageTop-" + imageTop + " imageWidth-" + imageWidth + " imageHeight-" + imageHeight /*+ UUID.randomUUID()*/ + ".png");
//            File outputFile = new File("E:\\Jae\\sibu\\需求1\\泰国\\VT-ENR-3.1-en-GB所有页图形图片\\" + "VT-ENR-3.1-en-GB"+"-Page" + pageIndex
//                    + " imageLeft-" + imageLeft + " imageTop-" + imageTop+ " imageWidth-" + imageWidth+ " imageHeight-" + imageHeight/*+ UUID.randomUUID()*/ + ".png");
