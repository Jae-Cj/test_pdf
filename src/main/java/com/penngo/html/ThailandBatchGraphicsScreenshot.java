...
public class ThailandBatchGraphicsScreenshot {
    public static void main(String[] args) {
        // 记录起始时间
        long l = System.currentTimeMillis();

        String filePath = "E:\\Jae\\sibu\\需求1\\泰国\\VT-ENR-3.3-en-GB.html"; // 替换HTML文件路径
    ...
    /**
     * 适用国家层级截图计算，不同国家使用的图不同，所以截取可能不完整，建议不同国家使用不同截图计算会比较完整
     *
     * @param pageIndex
     * @param imageLeft
     * @param imageTop
    ...
    private static void screenShot(int pageIndex, String imageLeft, String imageTop, String imageWidth, String imageHeight) {
        ...
        try {
            // 1. 加载PDF文档
            PDDocument document = PDDocument.load(new File("E:\\Jae\\sibu\\需求1\\泰国\\VT-ENR-3.3-en-GB.pdf"));
...
//            int pageIndex = 0;
            ...
            // 坐标计算系数【泰国】
            float leftCoefficient = -14f;
            float topCoefficient = -7f;
            float widthCoefficient = 35f;
            float heightCoefficient = 23f;
...
//                x = 0;// 可以截，但是缺角，原图也是缺角，left=0;top=4
            ...
            if (isScreenshotFlag) {
                ...
                // 7. 保存截图为PNG格式的图片文件
                File outputFile = new File("E:\\Jae\\sibu\\需求1\\泰国\\泰国所有图形截图\\泰国ENR3所有图形截图\\VT-ENR-3.3-en-GB所有页图形图片\\" + "VT-ENR-3.3-en-GB" + "-Page" + pageIndex
